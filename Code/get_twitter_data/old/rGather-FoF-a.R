...
names = read.csv("C:/Users/chris/Desktop/NS-FinalProject/half_data_a.csv", header = FALSE)
...
while (counter < 5000) {
  ...
  for (user in names$V2){
    print(user_pos)
    FoF = read.csv(paste("C:/Users/chris/Desktop/NS-FinalProject/half_data_a/", user, sep=""), header = TRUE)
    ...
    if (!(is.na(name)) && name < 325000000) {
      ...
      if (!(is.null(name %in% searched))){
        ...
        if (!(is.na(data))){
          write.csv(data, file = paste("C:/Users/chris/Desktop/NS-FinalProject/data_FoF_a/", name, ".csv", sep=""))
...
write.csv(searched, file = "C:/Users/chris/Desktop/NS-FinalProject/searched_a.csv")
